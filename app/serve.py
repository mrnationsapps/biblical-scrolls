...
.top{position:sticky;top:0;z-index:9;background:var(--card);border-bottom:1px solid var(--line);
  padding:8px 16px;display:flex;flex-direction:row;align-items:center;gap:12px;box-shadow:0 1px 4px rgba(0,0,0,.06);transition:transform .25s ease}
.top.hidden{transform:translateY(-105%)}
.topleft{display:flex;flex-direction:column;gap:3px;min-width:0;flex:1}
.topright{display:flex;align-items:center;gap:12px;flex:none;margin-left:auto}
...
<div class="top">
  <div class="topleft"><div class="logo" onclick="goFeed()">📜 Biblical Scrolls</div>
  ...
  <div class="topright">
    ...
    <div class="bell" id="bell" onclick="toggleNotifs()">🔔<span class="ncount" id="ncount"></span></div></div></div>
...
function toggleEng(){ENG=!ENG;const t=document.getElementById('tog');if(t)t.classList.toggle('on',ENG);const tp=document.getElementById('tocpanel');if(tp&&!ENG)tp.classList.remove('show');render();}
